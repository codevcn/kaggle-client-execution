...
Hệ thống còn hỗ trợ giao diện quản trị (Admin Panel) để theo dõi log và quản lý các cấu hình động, cũng như kết nối WebSocket thời gian thực cho các client cục bộ (local).
...
### 2.4. Kết Nối Thời Gian Thực (WebSocket Real-time)

- Hỗ trợ client cục bộ (Local Client) kết nối thông qua WebSockets.
- Nếu webhook trả về `text_data` chứa một đường dẫn URL hợp lệ, máy chủ sẽ lập tức phát sự kiện `handle` qua WebSocket đến tất cả các client đang kết nối để tiếp tục quy trình tải hoặc xử lý ở máy khách.

### 2.5. Cơ Chế Giữ Thức (Keep Alive)
...
3. **Phát Xử Lý Bất Đồng Bộ (Background Task):**
   ...
   - Nếu `text_data` chứa link: Đẩy tín hiệu URL xuống client local qua WebSocket.
...
- `SERVER_API_KEY`: Key cố định dùng bảo vệ Endpoints và WebSocket.
...
### 6.2. WebSockets (Dành cho Local Client)

- `WS /ws/local/{client_id}?token=...`: Endpoint để Local Script (như auto download, render tiếp theo trên máy cá nhân) có thể lắng nghe. Cần gắn Token qua query URL.

### 6.3. API Internal / Admin (Không Schema Docs)
