...
===================================
Kiểm tra thư mục LOCAL_INPUT_FOLDER_PATH có chứa các cặp file hợp lệ không.
...
LOCAL_INPUT_FOLDER_PATH: Path = ROOT_DIR / "media" / "others" / "music-videos"
...
def check_file_pairs(folder: Path) -> tuple[list[str], list[str]]:
    ...
    if not all_stems:
        return [], [f"Thư mục '{folder}' không chứa file audio hoặc .txt nào."]
    ...
    for stem in sorted(all_stems):
        ...
        if has_audio and has_txt:
            ...
            errors.append(
                f"File audio '{audio_stems[stem].name}' thiếu file txt tương ứng "
                f"(cần có: '{stem}.txt')"
            )
        else:  # has_txt and not has_audio
            errors.append(
                f"File txt '{txt_stems[stem].name}' thiếu file audio tương ứng "
...
def main() -> None:
    ...
    _log(f"   LOCAL_INPUT_FOLDER_PATH : {LOCAL_INPUT_FOLDER_PATH}")
    _log("=" * 60)

    valid_pairs, errors = check_file_pairs(LOCAL_INPUT_FOLDER_PATH)
