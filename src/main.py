...
# ─────────────────────────────────────────────
# Phân loại file theo loại media
# ─────────────────────────────────────────────
...
def collect_files_to_upload(
    local_data_input: str, file_type_to_upload: list[str]
) -> list[Path]:
    """
    Dò trong thư mục local_data_input (không đệ quy) và trả về
    danh sách các file khớp với file_type_to_upload.
    """
    ...
    # Tập hợp các đuôi file cần tìm
    allowed_extensions: set[str] = set()
    for ftype in file_type_to_upload:
        exts = FILE_TYPE_MAP.get(ftype.lower(), set())
        if not exts:
            logger.warning(f"  ⚠️  Loại file không nhận diện được: '{ftype}' — bỏ qua.")
        allowed_extensions |= exts

    if not allowed_extensions:
        logger.warning("  ⚠️  Không có đuôi file hợp lệ nào được xác định.")
        return []
    ...
    logger.info(f"  📂 Tìm thấy {len(files)} file phù hợp trong {folder}")
    return files
...
# ═══════════════════════════════════════════════════════════════════
# Phần 6 — Hàm chính: thực thi toàn bộ flows
# ═══════════════════════════════════════════════════════════════════
...
def run_all_flows(config: dict) -> None:
    ...
    for flow_idx, flow in enumerate(flows, start=1):
        ...
        file_type_to_upload: list[str] = flow.get("file_type_to_upload", [])
        ...
        logger.info(f"   file_type_to_upload: {file_type_to_upload}")
        ...
        if not file_type_to_upload:
            missing.append("file_type_to_upload")
        ...
        # ── Dò file cần upload ──────────────────────────────────
        files_to_process = collect_files_to_upload(
            local_data_input, file_type_to_upload
        )
        if not files_to_process:
            logger.warning(
                f"  ⚠️  Flow {flow_idx}: Không tìm thấy file nào phù hợp trong "
